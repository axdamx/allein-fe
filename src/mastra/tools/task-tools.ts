...
export const createTaskTool = createTool({
  id: 'create-task',
  description:
    'Create a task in the planner. Use when the user wants to schedule, plan, add a to-do, or create a task. Extract the title, description, priority, dates from the conversation context — do not ask the user for details they already mentioned.',
...
export const readTasksTool = createTool({
  ...
  execute: async ({ timeFrame, plannedDate, limit }, context) => {
    ...
    return {
      success: true,
      tasks: data.map((t: { id: string; title: string; description: string | null; status: string; priority: string; time_frame: string; planned_date: string | null; due_date: string | null; tags: string[] }) => ({
        id: t.id,
        title: t.title,
